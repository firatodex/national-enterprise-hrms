...
export async function fetchAll() {
  const [users, punches, loans, loanPayments, advances, monthCloses, config] =
    await Promise.all([
      ...
      supabase.from('punches').select('*').order('date').order('session'),
      supabase.from('loans').select('*').order('date'),
      supabase.from('loan_payments').select('*').order('created_at'),
      supabase.from('advances').select('*').order('date'),
      supabase.from('month_closes').select('*').order('year').order('month'),
