...
// Run this ONCE on Render shell: node migrate.js
// It will replace dummy seed data with your real Supabase data.
...
db.pragma('foreign_keys = OFF'); // OFF during migration to avoid ordering issues
...
// ─── STEP 2: Insert real users ─────────────────────────────────────────────
const insertUser = db.prepare(`
  INSERT INTO users (id, employee_code, full_name, password_hash, role, phone, joined_on, is_active, created_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
...
// ─── STEP 3: Insert real loans ─────────────────────────────────────────────
const insertLoan = db.prepare(`
  INSERT INTO loans (id, user_id, original_paise, issued_on, note, is_closed, issued_by, created_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?)
`);
...
const insertLoanTx = db.transaction(() => {
  for (const l of loans) insertLoan.run(...l);
});
insertLoanTx();
console.log(`Inserted ${loans.length} loans.`);

// ─── STEP 4: Update system_config with real values ─────────────────────────
db.prepare(`
  UPDATE system_config SET
    geofence_lat = 23.2363,
    geofence_lng = 72.5062,
    geofence_radius_meters = 200,
    lunch_start_minute = 780,
    lunch_end_minute = 840,
    ot_start_minute = 1050,
...
// ─── STEP 5: Re-enable foreign keys and verify ─────────────────────────────
